...
async function AdminDashboardData() {
  ...
  const similarMap = new Map<number, string>();

  for (const p of pendingProducts) {
    if (p.similarProjectId) {
      const match = allProducts.find((proj) => proj.id === p.similarProjectId);

      if (match) {
        similarMap.set(p.id, match.name);
      }
    }
  }
      ...
      <section className="my-12">
        ...
        <div className="space-y-4">
          ...
          {pendingProducts.length === 0 ? (
            ...
            <PendingProductsClient products={pendingProducts} similarMap={similarMap} />
          )}
        </div>
      </section>
    </>
